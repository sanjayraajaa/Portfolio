...
export const projects: Project[] = [
  {
    ...
    langs: [
      { name: "HTML", icon: <img src={htmlIcon} alt="HTML" className="w-5 h-5" /> },
      { name: "CSS", icon: <img src={cssIcon} alt="CSS" className="w-5 h-5" /> },
      { name: "JavaScript", icon: <img src={javascriptIcon} alt="JavaScript" className="w-5 h-5" /> },
    ],
    description:
      ...
      <div className="container mx-auto px-4 max-w-4xl">
        ...
        <div className="space-y-6 mx-2 sm:mx-4 md:mx-6">
          {projects.map((project) => (
            <Card
              key={project.id}
              className="rounded-lg shadow-md bg-gray-800 overflow-hidden hover:scale-102 transition-transform duration-300"
            >
              <div className="flex flex-col md:flex-row items-center justify-between p-4 space-y-4 md:space-y-0 md:space-x-4">
                {/* Left side: Image */}
                <div className="flex-shrink-0 w-full md:w-1/3 h-36 sm:h-48 rounded-lg overflow-hidden">
                  <img
